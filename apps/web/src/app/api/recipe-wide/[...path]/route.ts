...
async function proxyRecipeWide(request: Request, context: Context) {
  const { path } = await context.params;
  if (!path.length) {
    return NextResponse.json({ error: "不支持的配方宽表接口" }, { status: 404 });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: payload.detail ?? "后端配方宽表服务返回错误" },
...
export const GET = proxyRecipeWide;
export const POST = proxyRecipeWide;
