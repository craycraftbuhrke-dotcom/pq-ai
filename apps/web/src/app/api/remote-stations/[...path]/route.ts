...
const REMOTE_STATION_REQUEST_MAX_BYTES = 64 * 1024;
...
async function proxy(request: Request, context: Context) {
  ...
  if (!path.length) {
    return NextResponse.json({ error: "不支持的远程工作站操作" }, { status: 404 });
  }
  ...
  try {
    const body = request.method === "GET"
      ? undefined
      : (await readBoundedRequestBody(request, REMOTE_STATION_REQUEST_MAX_BYTES)).toString("utf-8");
    if (body) headers.set("Content-Type", request.headers.get("content-type") ?? "application/json");
    const response = await fetch(
      `${apiUrl}/remote-stations/${path.map(encodeURIComponent).join("/")}${new URL(request.url).search}`,
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: result.detail ?? "远程工作站服务返回错误" },
    ...
    if (clientStatus === 400 || clientStatus === 413) {
      return NextResponse.json(
        { error: error instanceof Error ? error.message : "远程工作站请求无效" },
        { status: clientStatus },
      );
    }
    return NextResponse.json(
      { error: isUpstreamTimeout(error) ? "远程工作站响应超时" : "无法连接后端远程工作站服务" },
