...
export function ProductionWorkspace() {
  ...
  return (
    <div className="page-stack">
      <header className="page-header"><div><span className="page-kicker">PRODUCTION TRACEABILITY</span><h1>生产实绩中心</h1><p>维护生产事件、五工序程序实绩、材料批次与实际参数，作为点位特征和 AI 建模的真实输入。</p></div><div className="page-actions"><BulkDataActions resourceKey={tab === "materials" ? "process.material-batches" : "process.production-runs"} resourceLabel={tab === "materials" ? "材料批次" : "生产事件"} disabled={loading || submitting} onImported={reload} onResult={bulkResult} /><button className="button button-secondary" onClick={() => void reload()}><RefreshCw className={loading ? "spin" : ""} /> 刷新实时数据</button></div></header>
      ...
      <div className="freshness">生产事件、工序实绩、材料批次和实绩参数采用停用、替换或追加治理；当前页面不提供物理删除。</div>
...
function ProductionModal({ modal, form, setForm, submit, close, submitting, factories, vehicleModels, colors, materials, versions, definitions, selectedRun }: { modal: NonNullable<Modal>; form: FormState; setForm: (form: FormState) => void; submit: (event: FormEvent<HTMLFormElement>) => void; close: () => void; submitting: boolean; factories: Resource[]; vehicleModels: Resource[]; colors: Resource[]; materials: Material[]; versions: ProgramVersion[]; definitions: Definition[]; selectedRun?: ProductionRun }) {
  // 组件仅在 modal 打开时被父组件渲染，因此 open=true 恒成立；close 已由父组件包一层 submitting 防抖。
  useModalDismiss({ open: true, onClose: close, busy: submitting });
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={submitting ? undefined : close}><section className="modal-card quality-modal" role="dialog" aria-modal="true" aria-labelledby="production-modal-title" onMouseDown={(event) => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">PRODUCTION DATA</span><h2 id="production-modal-title">{modal.record ? "编辑" : "新建"}{{ run: "生产事件", material: "材料批次", stage: "工序实绩", actual: "实际参数" }[modal.kind]}</h2></div><button className="icon-button" onClick={close} disabled={submitting} aria-label="关闭"><X aria-hidden="true" /></button></div><form onSubmit={submit}><div className="form-grid">{modal.kind === "run" ? <>{input("run_no", "生产事件编号", "text", true)}{input("body_no", "车身号")}{select("factory_id", "工厂", factories.map((item) => [item.id, `${item.code} / ${item.name}`]))}{select("vehicle_model_id", "车型", vehicleModels.map((item) => [item.id, `${item.code} / ${item.name}`]))}{select("color_id", "颜色", colors.map((item) => [item.id, `${item.code} / ${item.name}`]))}{input("shift", "班次")}{input("started_at", "开始时间", "datetime-local", true)}{input("completed_at", "完成时间", "datetime-local")}<TextArea keyName="context_values" label="追溯上下文 JSON（不进入 AI 特征）" form={form} setForm={setForm} /></> : null}{modal.kind === "material" ? <>{input("batch_no", "批次号", "text", true)}{input("material_code", "材料代码", "text", true)}{input("material_name", "材料名称", "text", true)}{select("material_type", "材料类型", [["MIDCOAT", "中涂"], ["BASECOAT", "色漆"], ["CLEARCOAT", "清漆"]])}{input("supplier", "供应商")}{input("viscosity", "粘度", "number")}{input("solid_ratio", "固含比", "number")}<TextArea keyName="coa_values" label="COA JSON（受范围策略校验）" form={form} setForm={setForm} /></> : null}{modal.kind === "stage" ? <><label className="form-field form-field-wide"><span>录入提示</span><div className="master-empty">当前工序：{currentStageLabel}，只展示匹配该工序的程序版本与{expectedMaterialLabel}材料批次。{selectedRun ? `当前生产事件：${selectedRun.run_no}` : "请先选定生产事件。"} </div></label>{select("process_stage", "工序", stages as [string, string][])}{select("program_version_id", "程序版本", filteredVersions.map((item) => [item.id, `${item.program_name} / ${item.version} / ${item.status}`]))}{filteredVersions.length === 0 ? <label className="form-field form-field-wide"><span>程序版本前置条件</span><div className="master-empty">{currentStageLabel} 暂无可用程序版本，请先到程序中心补齐对应工序的程序和版本，再回到此处录入实绩。</div></label> : null}<label className="form-field"><span>材料批次</span><select value={form.material_batch_id ?? ""} onChange={(event) => setForm({ ...form, material_batch_id: event.target.value })}><option value="">无</option>{filteredMaterials.map((item) => <option value={item.id} key={item.id}>{item.batch_no} / {item.material_name}</option>)}</select></label>{filteredMaterials.length === 0 ? <label className="form-field form-field-wide"><span>材料批次前置条件</span><div className="master-empty">当前工序期望 {expectedMaterialLabel} 材料，但暂未找到匹配批次；请先在材料批次页补录对应类型的受治理批次。</div></label> : null}{input("status", "状态", "text", true)}<TextArea keyName="actual_parameters" label="工序汇总参数 JSON（受范围策略校验）" form={form} setForm={setForm} /></> : null}{modal.kind === "actual" ? <>{select("parameter_code", "参数", definitions.map((item) => [item.code, `${item.name} / ${item.code}`]))}{input("actual_value", "实际值", "number", true)}{input("unit", "单位", "text", true)}{input("sampled_at", "采样时间", "datetime-local", true)}{input("source_system", "来源系统")}</> : null}</div><div className="modal-actions"><button type="button" className="button button-secondary" onClick={close} disabled={submitting}>取消</button><button className="button button-primary" disabled={submitting}>{submitting ? <LoaderCircle className="spin" aria-hidden="true" /> : null}{submitting ? "正在保存" : "保存到 MySQL"}</button></div></form></section></div>;
}
