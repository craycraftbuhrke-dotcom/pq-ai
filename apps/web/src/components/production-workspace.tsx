...
export function ProductionWorkspace() {
  ...
  return (
    <div className="page-stack">
      <header className="page-header"><div><span className="page-kicker">生产车身记录</span><h1>生产实绩中心</h1><p>维护生产事件、五工序程序实绩、材料批次与实际参数，记录每台车五个喷涂工位的实际参数。</p></div><div className="page-actions"><BulkDataActions resourceKey={tab === "materials" ? "process.material-batches" : "process.production-runs"} resourceLabel={tab === "materials" ? "材料批次" : "生产事件"} disabled={loading || submitting} onImported={reload} onResult={bulkResult} /><button className="button button-secondary" onClick={() => void reload()}><RefreshCw className={loading ? "spin" : ""} /> 刷新实时数据</button></div></header>
