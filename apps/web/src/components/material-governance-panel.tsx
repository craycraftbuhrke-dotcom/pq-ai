...
function renderFields(kind: Kind, form: FormState, setForm: (form: FormState) => void, resources: Record<Kind, Resource[]>, materials: MaterialBatch[]) {
  ...
  return [input("结果编号", "result_no", form, setForm, "text", true), select("材料批次", "material_batch_id", form, setForm, materials.map((item) => [item.id, `${item.batch_no} / ${item.material_name}`])), select("材料特性", "characteristic_definition_id", form, setForm, definitions), select("检测方法", "method_id", form, setForm, methods), input("结果值", "result_value", form, setForm, "number", true), input("单位", "unit", form, setForm, "text", true), input("检测时间", "tested_at", form, setForm, "datetime-local", true), input("检测人", "tested_by", form, setForm), input("来源文件 URI", "source_uri", form, setForm), textarea("原始结果 JSON", "raw_values", form, setForm), textarea("备注", "remark", form, setForm)];
}
