  ...
  return (
    <div className="measurement-governance material-governance">
      ...
      <section className="quality-analytics-stat-grid">
        ...
        <article><span>生效适用关系</span><strong>{summary?.active_applicabilities ?? 0} / {summary?.applicabilities ?? 0}</strong><small>按材料类型、工序和质量类型治理</small></article>
...
function renderFields(kind: Kind, form: FormState, setForm: (form: FormState) => void, resources: Record<Kind, Resource[]>, materials: MaterialBatch[]) {
  ...
  if (kind === "definitions") return [input("特性代码", "code", form, setForm, "text", true), input("特性名称", "name", form, setForm, "text", true), select("类别", "category", form, setForm, [["VISCOSITY_RHEOLOGY", "粘度/流变"], ["SOLIDS", "固含"], ["DENSITY", "密度"], ["PIGMENT_EFFECT", "颜料/效应"], ["LEVELING_SURFACE", "流平/表面"]]), input("标准单位", "canonical_unit", form, setForm, "text", true), input("适用质量类型（可多选，用逗号分隔）", "target_families", form, setForm, "text", true), select("状态", "status", form, setForm, [["ACTIVE", "生效"], ["RETIRED", "退役"]]), checkbox("允许进入模型", "is_model_feature", form, setForm), textarea("定义说明", "description", form, setForm)];
  if (kind === "methods") return [select("材料特性", "characteristic_definition_id", form, setForm, definitions), input("方法代码", "code", form, setForm, "text", true), input("方法名称", "name", form, setForm, "text", true), input("版本", "version", form, setForm, "text", true), input("方法类型", "method_type", form, setForm, "text", true), input("结果单位", "result_unit", form, setForm, "text", true), input("检测规程文件", "procedure_uri", form, setForm), select("状态", "status", form, setForm, [["ACTIVE", "生效"], ["RETIRED", "退役"]]), <label className="form-field form-field-wide" key="conditions"><span>检测条件明细</span><JsonObjectEditor value={String(form.conditions ?? "")} onChange={(value) => setForm({ ...form, conditions: value })} keyLabel="条件项" valueLabel="条件值" addLabel="新增条件" /></label>, textarea("备注", "remark", form, setForm)];
  if (kind === "specifications") return [select("材料代码", "material_code", form, setForm, Array.from(new Set(materials.map((item) => item.material_code))).map((code) => [code, code])), select("材料特性", "characteristic_definition_id", form, setForm, definitions), select("检测方法", "method_id", form, setForm, methods), input("规格版本", "version", form, setForm, "text", true), input("下限", "lower_limit", form, setForm, "number"), input("上限", "upper_limit", form, setForm, "number"), select("状态", "status", form, setForm, statusOptions), input("规格来源文件", "source_uri", form, setForm), input("有效开始", "effective_from", form, setForm, "datetime-local"), input("有效截止", "effective_to", form, setForm, "datetime-local"), input("审批人", "approved_by", form, setForm), textarea("备注", "remark", form, setForm)];
  if (kind === "applicabilities") return [select("材料特性", "characteristic_definition_id", form, setForm, definitions), select("材料类型", "material_type", form, setForm, [["MIDCOAT", "中涂"], ["BASECOAT", "色漆"], ["CLEARCOAT", "清漆"]]), select("执行工序", "process_stage", form, setForm, stageOptions), select("质量类型", "target_family", form, setForm, qualityOptions), checkbox("该工序与质量类型是否必须使用", "is_required", form, setForm), select("状态", "status", form, setForm, statusOptions), input("审批人", "approved_by", form, setForm), textarea("备注", "remark", form, setForm)];
  return [input("结果编号", "result_no", form, setForm, "text", true), select("材料批次", "material_batch_id", form, setForm, materials.map((item) => [item.id, `${item.batch_no} / ${item.material_name}`])), select("材料特性", "characteristic_definition_id", form, setForm, definitions), select("检测方法", "method_id", form, setForm, methods), input("结果值", "result_value", form, setForm, "number", true), input("单位", "unit", form, setForm, "text", true), input("检测时间", "tested_at", form, setForm, "datetime-local", true), input("检测人", "tested_by", form, setForm), input("来源文件", "source_uri", form, setForm), <label className="form-field form-field-wide" key="raw_values"><span>原始结果明细</span><JsonObjectEditor value={String(form.raw_values ?? "")} onChange={(value) => setForm({ ...form, raw_values: value })} keyLabel="原始项" valueLabel="原始值" addLabel="新增原始项" /></label>, textarea("备注", "remark", form, setForm)];
}
