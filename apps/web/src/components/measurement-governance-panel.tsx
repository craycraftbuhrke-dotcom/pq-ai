...
function textarea(label: string, key: string, form: FormState, setForm: (value: FormState) => void) {
  return <label className="form-field form-field-wide" key={key}><span>{label}</span><textarea rows={5} value={String(form[key] ?? "")} onChange={(event) => setForm({ ...form, [key]: event.target.value })} /></label>;
}
...
function renderFields(kind: Kind, form: FormState, setForm: (value: FormState) => void, resources: Record<Kind, GovernanceResource[]>) {
  ...
  if (kind === "references") return [input("参考件代码", "code", form, setForm, "text", true), input("参考件名称", "name", form, setForm, "text", true), select("质量类型", "quality_type", form, setForm, qualityOptions), input("序列号", "serial_no", form, setForm), input("证书编号", "certificate_no", form, setForm), input("有效开始", "valid_from", form, setForm, "datetime-local"), input("有效截止", "valid_until", form, setForm, "datetime-local"), select("状态", "status", form, setForm, [["ACTIVE", "在用"], ["EXPIRED", "过期"], ["RETIRED", "退役"]]), textarea("参考值 JSON", "reference_values", form, setForm)];
  if (kind === "calibrations") return [input("校准/检查编号", "calibration_no", form, setForm, "text", true), select("仪器", "instrument_id", form, setForm, resources.instruments.map((item) => [item.id, `${item.code} / ${item.name}`])), select("方法", "method_id", form, setForm, resources.methods.map((item) => [item.id, `${item.code}:${item.version}`]), false), select("参考件", "reference_standard_id", form, setForm, resources.references.map((item) => [item.id, `${item.code} / ${item.name}`]), false), input("校准/检查时间", "calibrated_at", form, setForm, "datetime-local", true), input("有效截止", "valid_until", form, setForm, "datetime-local", true), select("结果", "result", form, setForm, [["PASS", "通过"], ["FAIL", "失败"]]), input("执行人", "performed_by", form, setForm, "text", true), textarea("检查值 JSON", "check_values", form, setForm)];
  return [input("模板代码", "code", form, setForm, "text", true), input("模板名称", "name", form, setForm, "text", true), input("版本", "version", form, setForm, "text", true), select("仪器类型", "instrument_type", form, setForm, instrumentOptions), select("质量类型", "quality_type", form, setForm, qualityOptions), input("导出结构版本", "schema_version", form, setForm, "text", true), checkbox("模板生效", "is_active", form, setForm), textarea("字段映射 JSON", "field_mapping", form, setForm)];
}
