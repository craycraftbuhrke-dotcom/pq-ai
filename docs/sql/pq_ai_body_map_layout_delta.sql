...
-- layout_x/layout_y 0~1 range is enforced in application schemas/services (no DB CHECK; some MySQL proxies reject CHECK).
...
CREATE TABLE `measurement_point_layout` (
  ...
  `layout_x` DOUBLE NOT NULL COMMENT '相对底图归一化 X，范围 0~1，应用层校验',
  `layout_y` DOUBLE NOT NULL COMMENT '相对底图归一化 Y，范围 0~1，应用层校验',
