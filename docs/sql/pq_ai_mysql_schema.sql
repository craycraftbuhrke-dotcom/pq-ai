...
CREATE TABLE `actual_parameter` (
  `production_stage_run_id` VARCHAR(36) NOT NULL COMMENT '生产阶段实绩ID；应用层逻辑引用 production_stage_run.id',
  ...
  `parameter_code` VARCHAR(64) NOT NULL COMMENT '工艺参数编码',
  `actual_value` DECIMAL(18,6) NOT NULL COMMENT '实际值',
  `unit` VARCHAR(24) NOT NULL COMMENT '单位',
  `sampled_at` TIMESTAMP NOT NULL COMMENT '采样时间',
...
CREATE TABLE `api_key` (
  `user_id` VARCHAR(36) NOT NULL COMMENT '用户ID；应用层逻辑引用 app_user.id',
...
CREATE TABLE `audit_log` (
  ...
  `actor_username` VARCHAR(80) NOT NULL COMMENT '操作用户名',
  `action` VARCHAR(100) NOT NULL COMMENT '操作动作',
  `http_method` VARCHAR(12) NOT NULL COMMENT 'HTTP方法',
...
CREATE TABLE `brush` (
  `program_version_id` VARCHAR(36) NOT NULL COMMENT '程序版本ID；应用层逻辑引用 spray_program_version.id',
  `brush_no` VARCHAR(32) NOT NULL COMMENT '刷子号',
  `brush_table_no` VARCHAR(64) NOT NULL COMMENT '刷子表号',
...
CREATE TABLE `brush_parameter` (
  `brush_id` VARCHAR(36) NOT NULL COMMENT '刷子ID；应用层逻辑引用 brush.id',
  `parameter_definition_id` VARCHAR(36) NULL COMMENT '业务字段：parameter_definition_id；应用层逻辑引用 parameter_definition.id',
  `parameter_code` VARCHAR(64) NOT NULL COMMENT '工艺参数编码',
  `parameter_name` VARCHAR(120) NOT NULL COMMENT '工艺参数名称',
  `configured_value` DECIMAL(18,6) NOT NULL COMMENT '配置值',
  `unit` VARCHAR(24) NOT NULL COMMENT '单位',
...
CREATE TABLE `brush_point_contribution` (
  `brush_id` VARCHAR(36) NOT NULL COMMENT '刷子ID；应用层逻辑引用 brush.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `overlap_ratio` DECIMAL(18,6) NOT NULL COMMENT '业务字段：overlap_ratio',
  `contribution_weight` DECIMAL(18,6) NOT NULL COMMENT '业务字段：contribution_weight',
...
CREATE TABLE `closed_loop_evaluation` (
  `recommendation_id` VARCHAR(36) NOT NULL COMMENT '推荐ID；应用层逻辑引用 recommendation.id',
  `baseline_value` DECIMAL(18,6) NOT NULL COMMENT '业务字段：baseline_value',
  `verified_value` DECIMAL(18,6) NOT NULL COMMENT '业务字段：verified_value',
  `actual_improvement` DECIMAL(18,6) NOT NULL COMMENT '业务字段：actual_improvement',
  `is_effective` INT UNSIGNED NOT NULL COMMENT '业务字段：is_effective',
  `verified_at` TIMESTAMP NOT NULL COMMENT '业务字段：verified_at',
  `verified_by` VARCHAR(80) NOT NULL COMMENT '业务字段：verified_by',
  `conclusion` VARCHAR(2000) NULL COMMENT '业务字段：conclusion',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `color` (
  ...
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `color_type` VARCHAR(24) NOT NULL COMMENT '颜色类型',
...
CREATE TABLE `contribution_validation` (
  `contribution_version_id` VARCHAR(36) NOT NULL COMMENT '业务字段：contribution_version_id；应用层逻辑引用 point_contribution_version.id',
  `study_no` VARCHAR(64) NOT NULL COMMENT '业务字段：study_no',
  `target_family` VARCHAR(32) NOT NULL COMMENT '业务字段：target_family',
  `method` VARCHAR(32) NOT NULL COMMENT '方法',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态',
...
CREATE TABLE `controlled_trial` (
  `recommendation_id` VARCHAR(36) NOT NULL COMMENT '推荐ID；应用层逻辑引用 recommendation.id',
  `trial_no` VARCHAR(64) NOT NULL COMMENT '试验编号',
  `production_run_id` VARCHAR(36) NOT NULL COMMENT '生产事件ID；应用层逻辑引用 production_run.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `target_metric` VARCHAR(64) NOT NULL COMMENT '目标质量指标',
  `hypothesis` VARCHAR(2000) NOT NULL COMMENT '业务字段：hypothesis',
  `evidence_type` VARCHAR(32) NOT NULL COMMENT '证据类型',
  `expected_outcome` VARCHAR(2000) NOT NULL COMMENT '业务字段：expected_outcome',
  `risk_assessment` VARCHAR(2000) NOT NULL COMMENT '业务字段：risk_assessment',
  `rollback_plan` VARCHAR(2000) NOT NULL COMMENT '业务字段：rollback_plan',
  `sustained_observation_plan` VARCHAR(2000) NOT NULL COMMENT '业务字段：sustained_observation_plan',
  `constraint_evidence` JSON NOT NULL COMMENT '业务字段：constraint_evidence',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PLANNED' COMMENT '状态',
  `requested_by` VARCHAR(80) NOT NULL COMMENT '申请人',
  `requested_at` TIMESTAMP NOT NULL COMMENT '申请时间',
  `approved_by` VARCHAR(80) NULL COMMENT '审批人',
  `approved_at` TIMESTAMP NULL COMMENT '审批时间',
  `approval_comment` VARCHAR(2000) NULL COMMENT '业务字段：approval_comment',
  `started_at` TIMESTAMP NULL COMMENT '开始时间',
  `completed_at` TIMESTAMP NULL COMMENT '完成时间',
  `completion_summary` VARCHAR(2000) NULL COMMENT '业务字段：completion_summary',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `dataset_snapshot` (
  `dataset_code` VARCHAR(64) NOT NULL COMMENT '业务字段：dataset_code',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `target_metric` VARCHAR(64) NOT NULL COMMENT '目标质量指标',
  `feature_set_version` VARCHAR(64) NOT NULL COMMENT '业务字段：feature_set_version',
  `split_strategy` VARCHAR(48) NOT NULL COMMENT '业务字段：split_strategy',
  `group_key` VARCHAR(32) NOT NULL COMMENT '业务字段：group_key',
  `holdout_ratio` DECIMAL(18,6) NOT NULL COMMENT '业务字段：holdout_ratio',
  `status` VARCHAR(24) NOT NULL DEFAULT 'BUILT' COMMENT '状态',
  `sample_count` INT NOT NULL COMMENT '业务字段：sample_count',
  `group_count` INT NOT NULL COMMENT '编组数量',
  `train_sample_count` INT NOT NULL COMMENT '业务字段：train_sample_count',
  `validation_sample_count` INT NOT NULL COMMENT '业务字段：validation_sample_count',
  `train_group_count` INT NOT NULL COMMENT '业务字段：train_group_count',
  `validation_group_count` INT NOT NULL COMMENT '业务字段：validation_group_count',
  `cutoff_at` TIMESTAMP NULL COMMENT '业务字段：cutoff_at',
  `feature_names` JSON NOT NULL COMMENT '业务字段：feature_names',
  `lineage` JSON NOT NULL COMMENT '血缘JSON',
  `leakage_check` JSON NOT NULL COMMENT '业务字段：leakage_check',
  `built_at` TIMESTAMP NOT NULL COMMENT '业务字段：built_at',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `dataset_split_member` (
  `dataset_snapshot_id` VARCHAR(36) NOT NULL COMMENT '数据集快照ID；应用层逻辑引用 dataset_snapshot.id',
  `point_feature_snapshot_id` VARCHAR(36) NOT NULL COMMENT '业务字段：point_feature_snapshot_id；应用层逻辑引用 point_feature_snapshot.id',
  `production_run_id` VARCHAR(36) NOT NULL COMMENT '生产事件ID；应用层逻辑引用 production_run.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `target_measurement_id` VARCHAR(36) NOT NULL COMMENT '目标测量ID；应用层逻辑引用 quality_measurement.id',
  `group_value` VARCHAR(100) NOT NULL COMMENT '编组值',
  `split` VARCHAR(24) NOT NULL COMMENT '业务字段：split',
  `target_value` DECIMAL(18,6) NOT NULL COMMENT '目标值',
  `feature_values` JSON NOT NULL COMMENT '特征值JSON',
  `occurred_at` TIMESTAMP NOT NULL COMMENT '发生时间',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `diagnosis_result` (
  ...
  `production_run_id` VARCHAR(36) NOT NULL COMMENT '生产事件ID；应用层逻辑引用 production_run.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `metric_code` VARCHAR(64) NOT NULL COMMENT '质量指标编码',
  `summary` VARCHAR(2000) NOT NULL COMMENT '业务字段：summary',
  `factor_contributions` JSON NOT NULL COMMENT '业务字段：factor_contributions',
  `confidence` DECIMAL(18,6) NOT NULL COMMENT '业务字段：confidence',
...
CREATE TABLE `durr_application_controller` (
  `factory_id` VARCHAR(36) NOT NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `code` VARCHAR(64) NOT NULL COMMENT '编码',
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `model` VARCHAR(120) NOT NULL COMMENT '模型',
  `serial_no` VARCHAR(120) NOT NULL COMMENT '业务字段：serial_no',
...
CREATE TABLE `eng_knowledge_entry` (
  `entry_code` VARCHAR(64) NOT NULL COMMENT '业务字段：entry_code',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `title` VARCHAR(180) NOT NULL COMMENT '业务字段：title',
  `category` VARCHAR(48) NOT NULL COMMENT '业务字段：category',
  `target_quality_type` VARCHAR(32) NULL COMMENT '目标质量类型',
  `metric_code` VARCHAR(64) NULL COMMENT '质量指标编码',
  `symptom_pattern` VARCHAR(2000) NOT NULL COMMENT '业务字段：symptom_pattern',
  `diagnosis_rule` VARCHAR(2000) NOT NULL COMMENT '业务字段：diagnosis_rule',
  `recommended_checks` JSON NOT NULL COMMENT '业务字段：recommended_checks',
  `related_parameters` JSON NOT NULL COMMENT '业务字段：related_parameters',
...
CREATE TABLE `factory_vehicle_model` (
  `factory_id` VARCHAR(36) NOT NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `vehicle_model_id` VARCHAR(36) NOT NULL COMMENT '车型ID；应用层逻辑引用 vehicle_model.id',
  `is_active` INT UNSIGNED NOT NULL DEFAULT 1 COMMENT '是否启用',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `file_import_job` (
  `import_no` VARCHAR(64) NOT NULL COMMENT '业务字段：import_no',
  `profile_id` VARCHAR(36) NOT NULL COMMENT '业务字段：profile_id；应用层逻辑引用 file_import_profile.id',
  `domain_type` VARCHAR(48) NOT NULL COMMENT '业务字段：domain_type',
  `source_filename` VARCHAR(240) NOT NULL COMMENT '业务字段：source_filename',
  `source_uri` VARCHAR(500) NULL COMMENT '来源文件URI',
  `source_checksum` VARCHAR(128) NULL COMMENT '业务字段：source_checksum',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PREVIEWED' COMMENT '状态',
...
CREATE TABLE `file_import_profile` (
  ...
  `target_resource` VARCHAR(80) NOT NULL COMMENT '业务字段：target_resource',
  `field_mapping` JSON NOT NULL COMMENT '业务字段：field_mapping',
  `required_fields` JSON NOT NULL COMMENT '业务字段：required_fields',
...
CREATE TABLE `integration_endpoint` (
  `code` VARCHAR(64) NOT NULL COMMENT '编码',
  `name` VARCHAR(160) NOT NULL COMMENT '名称',
  `system_type` VARCHAR(32) NOT NULL COMMENT '业务字段：system_type',
...
CREATE TABLE `integration_event` (
  `event_no` VARCHAR(80) NOT NULL COMMENT '事件编号',
  `endpoint_id` VARCHAR(36) NOT NULL COMMENT '集成端点ID；应用层逻辑引用 integration_endpoint.id',
  `source_event_id` VARCHAR(160) NOT NULL COMMENT '来源事件ID',
  ...
  `last_error` VARCHAR(2000) NULL COMMENT '业务字段：last_error',
...
CREATE TABLE `mat_char_applicability` (
  `characteristic_definition_id` VARCHAR(36) NOT NULL COMMENT '业务字段：characteristic_definition_id；应用层逻辑引用 mat_char_definition.id',
  `material_type` VARCHAR(24) NOT NULL COMMENT '材料类型',
  `process_stage` VARCHAR(32) NOT NULL COMMENT '喷涂执行阶段',
  `target_family` VARCHAR(32) NOT NULL COMMENT '业务字段：target_family',
...
CREATE TABLE `mat_char_definition` (
  `code` VARCHAR(64) NOT NULL COMMENT '编码',
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `category` VARCHAR(32) NOT NULL COMMENT '业务字段：category',
  `canonical_unit` VARCHAR(24) NOT NULL COMMENT '业务字段：canonical_unit',
  `target_families` JSON NOT NULL COMMENT '业务字段：target_families',
...
CREATE TABLE `material_batch_test_result` (
  `result_no` VARCHAR(80) NOT NULL COMMENT '结果编号',
  `material_batch_id` VARCHAR(36) NOT NULL COMMENT '材料批次ID；应用层逻辑引用 material_batch.id',
  `characteristic_definition_id` VARCHAR(36) NOT NULL COMMENT '业务字段：characteristic_definition_id；应用层逻辑引用 mat_char_definition.id',
  `method_id` VARCHAR(36) NOT NULL COMMENT '方法ID；应用层逻辑引用 material_test_method.id',
  `specification_id` VARCHAR(36) NULL COMMENT '业务字段：specification_id；应用层逻辑引用 material_specification.id',
  `result_value` DECIMAL(18,6) NOT NULL COMMENT '结果值',
  `unit` VARCHAR(24) NOT NULL COMMENT '单位',
  `tested_at` TIMESTAMP NOT NULL COMMENT '业务字段：tested_at',
...
CREATE TABLE `material_test_method` (
  ...
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `method_type` VARCHAR(64) NOT NULL COMMENT '方法类型',
  `result_unit` VARCHAR(24) NOT NULL COMMENT '业务字段：result_unit',
...
CREATE TABLE `measurement_calibration_record` (
  `calibration_no` VARCHAR(64) NOT NULL COMMENT '校准编号',
  `instrument_id` VARCHAR(36) NOT NULL COMMENT '测量仪器ID；应用层逻辑引用 measurement_instrument.id',
  ...
  `calibrated_at` TIMESTAMP NOT NULL COMMENT '业务字段：calibrated_at',
  `valid_until` TIMESTAMP NOT NULL COMMENT '业务字段：valid_until',
  `result` VARCHAR(24) NOT NULL COMMENT '结果',
  `performed_by` VARCHAR(80) NOT NULL COMMENT '业务字段：performed_by',
...
CREATE TABLE `measurement_group` (
  ...
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `vehicle_model_id` VARCHAR(36) NOT NULL COMMENT '车型ID；应用层逻辑引用 vehicle_model.id',
  `quality_type` VARCHAR(32) NOT NULL COMMENT '质量指标类型',
...
CREATE TABLE `measurement_group_point` (
  `measurement_group_id` VARCHAR(36) NOT NULL COMMENT '测量编组ID；应用层逻辑引用 measurement_group.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
...
CREATE TABLE `measurement_import_profile` (
  ...
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `instrument_type` VARCHAR(32) NOT NULL COMMENT '仪器类型',
  `quality_type` VARCHAR(32) NOT NULL COMMENT '质量指标类型',
  `schema_version` VARCHAR(64) NOT NULL COMMENT '业务字段：schema_version',
  `field_mapping` JSON NOT NULL COMMENT '业务字段：field_mapping',
  `is_active` INT UNSIGNED NOT NULL DEFAULT 1 COMMENT '是否启用',
  `remark` VARCHAR(2000) NULL COMMENT '备注',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
...
CREATE TABLE `measurement_instrument` (
  `code` VARCHAR(64) NOT NULL COMMENT '编码',
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `manufacturer` VARCHAR(80) NOT NULL COMMENT '业务字段：manufacturer',
  `model` VARCHAR(120) NOT NULL COMMENT '模型',
  `instrument_type` VARCHAR(32) NOT NULL COMMENT '仪器类型',
  `serial_no` VARCHAR(120) NOT NULL COMMENT '业务字段：serial_no',
  `firmware_version` VARCHAR(64) NULL COMMENT '业务字段：firmware_version',
  `supported_quality_types` JSON NOT NULL COMMENT '业务字段：supported_quality_types',
...
CREATE TABLE `measurement_method` (
  ...
  `is_active` INT UNSIGNED NOT NULL DEFAULT 1 COMMENT '是否启用',
  `instructions` VARCHAR(2000) NULL COMMENT '业务字段：instructions',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `measurement_msa_study` (
  ...
  `sample_count` INT NOT NULL COMMENT '业务字段：sample_count',
  `operator_count` INT NOT NULL COMMENT '业务字段：operator_count',
  `repeat_count` INT NOT NULL COMMENT '业务字段：repeat_count',
  ...
  `study_at` TIMESTAMP NOT NULL COMMENT '业务字段：study_at',
  `approved_by` VARCHAR(80) NULL COMMENT '审批人',
  `approved_at` TIMESTAMP NULL COMMENT '审批时间',
...
CREATE TABLE `measurement_point` (
  ...
  `name` VARCHAR(120) NOT NULL COMMENT '名称',
  `vehicle_model_id` VARCHAR(36) NOT NULL COMMENT '车型ID；应用层逻辑引用 vehicle_model.id',
  `part_id` VARCHAR(36) NOT NULL COMMENT '零件ID；应用层逻辑引用 part.id',
  `point_type` VARCHAR(32) NOT NULL DEFAULT 'QUALITY' COMMENT '点位类型',
  `region` VARCHAR(80) NULL COMMENT '业务字段：region',
  `quality_types` JSON NOT NULL COMMENT '业务字段：quality_types',
...
CREATE TABLE `measurement_repeat_reading` (
  `measurement_id` VARCHAR(36) NOT NULL COMMENT '测量ID；应用层逻辑引用 quality_measurement.id',
  `repeat_no` INT NOT NULL COMMENT '业务字段：repeat_no',
  `metric_code` VARCHAR(64) NOT NULL COMMENT '质量指标编码',
  `raw_value` DECIMAL(18,6) NOT NULL COMMENT '原始值',
...
CREATE TABLE `model_acceptance_decision` (
  `model_version_id` VARCHAR(36) NOT NULL COMMENT '模型版本ID；应用层逻辑引用 model_version.id',
  `dataset_snapshot_id` VARCHAR(36) NOT NULL COMMENT '数据集快照ID；应用层逻辑引用 dataset_snapshot.id',
  `decision` VARCHAR(24) NOT NULL COMMENT '业务字段：decision',
  `criteria` JSON NOT NULL COMMENT '业务字段：criteria',
  `checks` JSON NOT NULL COMMENT '业务字段：checks',
  `decided_by` VARCHAR(80) NOT NULL COMMENT '业务字段：decided_by',
  `decided_at` TIMESTAMP NOT NULL COMMENT '业务字段：decided_at',
  `comment` VARCHAR(2000) NULL COMMENT '协作记录',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `model_acceptance_policy` (
  `policy_code` VARCHAR(64) NOT NULL COMMENT '业务字段：policy_code',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `factory_id` VARCHAR(36) NOT NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `target_metric` VARCHAR(64) NOT NULL COMMENT '目标质量指标',
  `policy_type` VARCHAR(24) NOT NULL DEFAULT 'FACTORY_APPROVED' COMMENT '业务字段：policy_type',
  `max_validation_rmse` DECIMAL(18,6) NOT NULL COMMENT '业务字段：max_validation_rmse',
  `min_validation_r2` DECIMAL(18,6) NOT NULL COMMENT '业务字段：min_validation_r2',
  `min_train_groups` INT NOT NULL COMMENT '业务字段：min_train_groups',
  `min_validation_groups` INT NOT NULL COMMENT '业务字段：min_validation_groups',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态',
  `source_uri` VARCHAR(500) NOT NULL COMMENT '来源文件URI',
  `approved_by` VARCHAR(80) NULL COMMENT '审批人',
  `approved_at` TIMESTAMP NULL COMMENT '审批时间',
  `remark` VARCHAR(2000) NULL COMMENT '备注',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
...
CREATE TABLE `model_applicability_scope` (
  ...
  `factory_id` VARCHAR(36) NOT NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `vehicle_model_id` VARCHAR(36) NOT NULL COMMENT '车型ID；应用层逻辑引用 vehicle_model.id',
  `color_id` VARCHAR(36) NOT NULL COMMENT '颜色ID；应用层逻辑引用 color.id',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PENDING' COMMENT '状态',
...
CREATE TABLE `model_artifact` (
  `model_version_id` VARCHAR(36) NOT NULL COMMENT '模型版本ID；应用层逻辑引用 model_version.id',
  `artifact_type` VARCHAR(48) NOT NULL COMMENT '业务字段：artifact_type',
  `artifact_uri` VARCHAR(500) NOT NULL COMMENT '业务字段：artifact_uri',
  `storage_backend` VARCHAR(32) NOT NULL DEFAULT 'MYSQL' COMMENT '业务字段：storage_backend',
  `payload_hash` VARCHAR(64) NOT NULL COMMENT '业务字段：payload_hash',
  `metadata_payload` JSON NOT NULL COMMENT '业务字段：metadata_payload',
  `status` VARCHAR(24) NOT NULL DEFAULT 'REGISTERED' COMMENT '状态',
  `created_by` VARCHAR(80) NOT NULL DEFAULT 'system' COMMENT '创建人',
  `registered_at` TIMESTAMP NOT NULL COMMENT '业务字段：registered_at',
  `remark` VARCHAR(2000) NULL COMMENT '备注',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `model_explanation` (
  `model_version_id` VARCHAR(36) NOT NULL COMMENT '模型版本ID；应用层逻辑引用 model_version.id',
  `prediction_result_id` VARCHAR(36) NULL COMMENT '预测结果ID；应用层逻辑引用 prediction_result.id',
  `explanation_type` VARCHAR(32) NOT NULL COMMENT '业务字段：explanation_type',
  `target_metric` VARCHAR(64) NOT NULL COMMENT '目标质量指标',
  `feature_impacts` JSON NOT NULL COMMENT '业务字段：feature_impacts',
  `sensitivity_grid` JSON NULL COMMENT '业务字段：sensitivity_grid',
  `uncertainty` JSON NULL COMMENT '业务字段：uncertainty',
  `generated_at` TIMESTAMP NOT NULL COMMENT '业务字段：generated_at',
...
CREATE TABLE `model_ood_policy` (
  `model_version_id` VARCHAR(36) NOT NULL COMMENT '模型版本ID；应用层逻辑引用 model_version.id',
  `max_abs_standardized_shift` DECIMAL(18,6) NOT NULL COMMENT '业务字段：max_abs_standardized_shift',
  `max_outlier_feature_ratio` DECIMAL(18,6) NOT NULL COMMENT '业务字段：max_outlier_feature_ratio',
  `min_feature_completeness` DECIMAL(18,6) NOT NULL COMMENT '业务字段：min_feature_completeness',
  `action` VARCHAR(24) NOT NULL DEFAULT 'BLOCK' COMMENT '操作动作',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PENDING' COMMENT '状态',
  `approved_by` VARCHAR(80) NULL COMMENT '审批人',
  `approved_at` TIMESTAMP NULL COMMENT '审批时间',
  `remark` VARCHAR(2000) NULL COMMENT '备注',
...
CREATE TABLE `model_validation_fold` (
  `model_version_id` VARCHAR(36) NOT NULL COMMENT '模型版本ID；应用层逻辑引用 model_version.id',
  `dataset_snapshot_id` VARCHAR(36) NOT NULL COMMENT '数据集快照ID；应用层逻辑引用 dataset_snapshot.id',
  `validation_axis` VARCHAR(48) NOT NULL COMMENT '业务字段：validation_axis',
  `fold_key` VARCHAR(120) NOT NULL COMMENT '业务字段：fold_key',
  `train_sample_count` INT NOT NULL COMMENT '业务字段：train_sample_count',
  `validation_sample_count` INT NOT NULL COMMENT '业务字段：validation_sample_count',
  `train_group_count` INT NOT NULL COMMENT '业务字段：train_group_count',
  `validation_group_count` INT NOT NULL COMMENT '业务字段：validation_group_count',
  `metrics` JSON NOT NULL COMMENT '业务字段：metrics',
  `status` VARCHAR(32) NOT NULL COMMENT '状态',
  `evaluated_at` TIMESTAMP NOT NULL COMMENT '业务字段：evaluated_at',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `model_version` (
  ...
  `model_payload` JSON NOT NULL COMMENT '模型载荷',
  `evaluation_metrics` JSON NOT NULL COMMENT '业务字段：evaluation_metrics',
...
CREATE TABLE `parameter_constraint_source` (
  `parameter_definition_id` VARCHAR(36) NOT NULL COMMENT '业务字段：parameter_definition_id；应用层逻辑引用 parameter_definition.id',
  `factory_id` VARCHAR(36) NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `process_stage` VARCHAR(32) NULL COMMENT '喷涂执行阶段',
  `constraint_code` VARCHAR(96) NOT NULL COMMENT '业务字段：constraint_code',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `source_type` VARCHAR(32) NOT NULL COMMENT '来源类型',
  `source_uri` VARCHAR(500) NULL COMMENT '来源文件URI',
  `lower_limit` DECIMAL(18,6) NOT NULL COMMENT '下限',
  `upper_limit` DECIMAL(18,6) NOT NULL COMMENT '上限',
  `unit` VARCHAR(24) NOT NULL COMMENT '单位',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态',
  `effective_from` TIMESTAMP NULL COMMENT '业务字段：effective_from',
  `effective_to` TIMESTAMP NULL COMMENT '业务字段：effective_to',
...
CREATE TABLE `path_segment_execution` (
  `device_execution_id` VARCHAR(36) NOT NULL COMMENT '设备执行ID；应用层逻辑引用 production_device_execution.id',
  `path_segment_id` VARCHAR(36) NOT NULL COMMENT '业务字段：path_segment_id；应用层逻辑引用 trajectory_path_segment.id',
...
CREATE TABLE `point_contribution_entry` (
  ...
  `source_key` VARCHAR(100) NOT NULL COMMENT '业务字段：source_key',
  `overlap_ratio` DECIMAL(18,6) NOT NULL COMMENT '业务字段：overlap_ratio',
  `contribution_weight` DECIMAL(18,6) NOT NULL COMMENT '业务字段：contribution_weight',
  `validation_score` DECIMAL(18,6) NULL COMMENT '业务字段：validation_score',
...
CREATE TABLE `point_feature_snapshot` (
  ...
  `feature_values` JSON NOT NULL COMMENT '特征值JSON',
  `lineage` JSON NOT NULL COMMENT '血缘JSON',
  `completeness_score` DECIMAL(18,6) NOT NULL COMMENT '业务字段：completeness_score',
  `generated_at` TIMESTAMP NOT NULL COMMENT '业务字段：generated_at',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `prediction_result` (
  ...
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `metric_code` VARCHAR(64) NOT NULL COMMENT '质量指标编码',
  `predicted_value` DECIMAL(18,6) NOT NULL COMMENT '业务字段：predicted_value',
  ...
  `predicted_at` TIMESTAMP NOT NULL COMMENT '业务字段：predicted_at',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `process_route_applicability` (
  `process_route_id` VARCHAR(36) NOT NULL COMMENT '业务字段：process_route_id；应用层逻辑引用 process_route.id',
...
CREATE TABLE `production_device_execution` (
  `production_stage_run_id` VARCHAR(36) NOT NULL COMMENT '生产阶段实绩ID；应用层逻辑引用 production_stage_run.id',
  `device_configuration_id` VARCHAR(36) NOT NULL COMMENT '业务字段：device_configuration_id；应用层逻辑引用 program_device_configuration.id',
  `trajectory_program_id` VARCHAR(36) NOT NULL COMMENT '轨迹程序ID；应用层逻辑引用 trajectory_program.id',
  `executed_checksum` VARCHAR(128) NOT NULL COMMENT '业务字段：executed_checksum',
  `started_at` TIMESTAMP NULL COMMENT '开始时间',
  `completed_at` TIMESTAMP NULL COMMENT '完成时间',
  `status` VARCHAR(24) NOT NULL DEFAULT 'COMPLETED' COMMENT '状态',
...
CREATE TABLE `production_run` (
  `run_no` VARCHAR(64) NOT NULL COMMENT '生产事件编号',
...
CREATE TABLE `program_device_configuration` (
  `program_version_id` VARCHAR(36) NOT NULL COMMENT '程序版本ID；应用层逻辑引用 spray_program_version.id',
  `robot_id` VARCHAR(36) NOT NULL COMMENT '机器人ID；应用层逻辑引用 durr_robot.id',
  `atomizer_id` VARCHAR(36) NOT NULL COMMENT '业务字段：atomizer_id；应用层逻辑引用 durr_rotary_atomizer.id',
  `controller_id` VARCHAR(36) NOT NULL COMMENT '业务字段：controller_id；应用层逻辑引用 durr_application_controller.id',
  `configuration_version` VARCHAR(32) NOT NULL COMMENT '业务字段：configuration_version',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态',
  `source_uri` VARCHAR(500) NULL COMMENT '来源文件URI',
  `approved_by` VARCHAR(80) NULL COMMENT '审批人',
  `approved_at` TIMESTAMP NULL COMMENT '审批时间',
...
CREATE TABLE `program_rollback_execution` (
  `rollback_no` VARCHAR(64) NOT NULL COMMENT '业务字段：rollback_no',
  `recommendation_id` VARCHAR(36) NOT NULL COMMENT '推荐ID；应用层逻辑引用 recommendation.id',
  `controlled_trial_id` VARCHAR(36) NOT NULL COMMENT '受控试验ID；应用层逻辑引用 controlled_trial.id',
  ...
  `rollback_reason` VARCHAR(2000) NOT NULL COMMENT '业务字段：rollback_reason',
  `execution_note` VARCHAR(2000) NULL COMMENT '业务字段：execution_note',
  `executed_by` VARCHAR(80) NOT NULL COMMENT '业务字段：executed_by',
  `executed_at` TIMESTAMP NOT NULL COMMENT '业务字段：executed_at',
  `status` VARCHAR(24) NOT NULL DEFAULT 'EXECUTED' COMMENT '状态',
  `action_snapshot` JSON NOT NULL COMMENT '业务字段：action_snapshot',
  `verified_by` VARCHAR(80) NULL COMMENT '业务字段：verified_by',
  `verified_at` TIMESTAMP NULL COMMENT '业务字段：verified_at',
  `verification_comment` VARCHAR(2000) NULL COMMENT '业务字段：verification_comment',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `quality_issue_comment` (
  `task_id` VARCHAR(36) NOT NULL COMMENT '工单ID；应用层逻辑引用 quality_issue_task.id',
  `author` VARCHAR(80) NOT NULL COMMENT '业务字段：author',
  `role` VARCHAR(64) NULL COMMENT '业务字段：role',
  `comment_type` VARCHAR(32) NOT NULL DEFAULT 'COMMENT' COMMENT '协作记录类型',
  `body` VARCHAR(2000) NOT NULL COMMENT '业务字段：body',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `quality_issue_task` (
  ...
  `status` VARCHAR(24) NOT NULL DEFAULT 'OPEN' COMMENT '状态',
  ...
  `problem_statement` VARCHAR(2000) NOT NULL COMMENT '业务字段：problem_statement',
  `hypothesis` VARCHAR(2000) NULL COMMENT '业务字段：hypothesis',
  `suspected_cause` VARCHAR(2000) NULL COMMENT '业务字段：suspected_cause',
  `conclusion` VARCHAR(2000) NULL COMMENT '业务字段：conclusion',
  `causality_status` VARCHAR(32) NOT NULL DEFAULT 'CORRELATION_ONLY' COMMENT '业务字段：causality_status',
  ...
  `ai_summary` VARCHAR(2000) NULL COMMENT '业务字段：ai_summary',
...
CREATE TABLE `quality_measurement` (
  `data_no` VARCHAR(64) NOT NULL COMMENT '质量数据编号',
  `production_run_id` VARCHAR(36) NOT NULL COMMENT '生产事件ID；应用层逻辑引用 production_run.id',
...
CREATE TABLE `quality_metric_value` (
  `measurement_id` VARCHAR(36) NOT NULL COMMENT '测量ID；应用层逻辑引用 quality_measurement.id',
  `metric_code` VARCHAR(64) NOT NULL COMMENT '质量指标编码',
  `metric_name` VARCHAR(120) NOT NULL COMMENT '质量指标名称',
  `raw_value` DECIMAL(18,6) NOT NULL COMMENT '原始值',
  `corrected_value` DECIMAL(18,6) NULL COMMENT '修正值',
  `unit` VARCHAR(24) NULL COMMENT '单位',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
...
CREATE TABLE `recommendation` (
  `recommendation_no` VARCHAR(64) NOT NULL COMMENT '推荐编号',
  `production_run_id` VARCHAR(36) NOT NULL COMMENT '生产事件ID；应用层逻辑引用 production_run.id',
  `measurement_point_id` VARCHAR(36) NOT NULL COMMENT '测量点ID；应用层逻辑引用 measurement_point.id',
  `target_quality_type` VARCHAR(32) NOT NULL COMMENT '目标质量类型',
  `target_metric` VARCHAR(64) NOT NULL COMMENT '目标质量指标',
  `diagnosis_summary` VARCHAR(2000) NOT NULL COMMENT '业务字段：diagnosis_summary',
  `predicted_improvement` DECIMAL(18,6) NOT NULL COMMENT '业务字段：predicted_improvement',
  `confidence` DECIMAL(18,6) NOT NULL COMMENT '业务字段：confidence',
  `status` VARCHAR(24) NOT NULL DEFAULT 'PENDING' COMMENT '状态',
...
CREATE TABLE `recommendation_action` (
  ...
  `parameter_code` VARCHAR(64) NOT NULL COMMENT '工艺参数编码',
  `parameter_name` VARCHAR(120) NOT NULL COMMENT '工艺参数名称',
  `current_value` DECIMAL(18,6) NOT NULL COMMENT '业务字段：current_value',
  `recommended_value` DECIMAL(18,6) NOT NULL COMMENT '业务字段：recommended_value',
...
CREATE TABLE `role_permission` (
  `role_id` VARCHAR(36) NOT NULL COMMENT '角色ID；应用层逻辑引用 role.id',
  `permission_id` VARCHAR(36) NOT NULL COMMENT '权限ID；应用层逻辑引用 permission.id',
  `id` VARCHAR(36) NOT NULL COMMENT '主键ID',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `spray_program` (
  `program_code` VARCHAR(64) NOT NULL COMMENT '喷涂程序编号',
  `name` VARCHAR(160) NOT NULL COMMENT '名称',
  `factory_id` VARCHAR(36) NOT NULL COMMENT '工厂ID；应用层逻辑引用 factory.id',
  `process_stage` VARCHAR(32) NOT NULL COMMENT '喷涂执行阶段',
...
CREATE TABLE `spray_program_version` (
  `spray_program_id` VARCHAR(36) NOT NULL COMMENT '喷涂程序ID；应用层逻辑引用 spray_program.id',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `status` VARCHAR(24) NOT NULL DEFAULT 'DRAFT' COMMENT '状态',
...
CREATE TABLE `supplier_mat_issue` (
  ...
  `containment_action` VARCHAR(2000) NULL COMMENT '业务字段：containment_action',
  `supplier_response` VARCHAR(2000) NULL COMMENT '业务字段：supplier_response',
  `resolution` VARCHAR(2000) NULL COMMENT '业务字段：resolution',
...
CREATE TABLE `trajectory_path_segment` (
  `trajectory_program_id` VARCHAR(36) NOT NULL COMMENT '轨迹程序ID；应用层逻辑引用 trajectory_program.id',
  `segment_no` INT NOT NULL COMMENT '路径段编号',
  `name` VARCHAR(160) NOT NULL COMMENT '名称',
  `brush_id` VARCHAR(36) NULL COMMENT '刷子ID；应用层逻辑引用 brush.id',
  `part_id` VARCHAR(36) NULL COMMENT '零件ID；应用层逻辑引用 part.id',
...
CREATE TABLE `trajectory_program` (
  ...
  `name` VARCHAR(160) NOT NULL COMMENT '名称',
  `version` VARCHAR(32) NOT NULL COMMENT '版本号',
  `checksum` VARCHAR(128) NOT NULL COMMENT '业务字段：checksum',
...
CREATE TABLE `trajectory_segment_geometry` (
  `path_segment_id` VARCHAR(36) NOT NULL COMMENT '业务字段：path_segment_id；应用层逻辑引用 trajectory_path_segment.id',
  `geometry_version` VARCHAR(32) NOT NULL COMMENT '业务字段：geometry_version',
...
CREATE TABLE `user_session` (
  `user_id` VARCHAR(36) NOT NULL COMMENT '用户ID；应用层逻辑引用 app_user.id',
  `token_hash` VARCHAR(64) NOT NULL COMMENT '会话令牌哈希',
