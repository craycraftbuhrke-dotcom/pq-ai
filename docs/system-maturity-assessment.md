...
| AI modeling | 89% | Equal production/manual sources, immutable datasets, leakage-safe temporal inner model selection, untouched final validation, Ridge/Elastic Net comparison, validation-based intervals, multi-axis validation, artifact hashes, factory acceptance policies, exact applicability, OOD blocking, persisted prediction/diagnosis/recommendation and drift | Real factory policy configuration, asynchronous training workers, calibrated uncertainty, nonlinear models, governed external artifact storage, and causal evidence |
...
- Production point snapshots and validated manual wide-table samples have equal status. Manual samples now participate in training, temporal holdout and independent-group validation instead of disappearing from production-context joins.
- Automatic training compares Ridge and Elastic Net candidates only inside temporally ordered training folds. The final validation holdout is not used for selection, and its RMSE drives the reported prediction interval.
...
- Point use resolves only active models matching the current factory/model/color and feature distribution. Recommendation targets default to the most specific active quality standard, while overlapping old active models retire on activation.
- Residual work: configure real factory-approved policies, add asynchronous training workers, calibrated uncertainty, governed external artifact storage, nonlinear model families, and real factory-run acceptance.
