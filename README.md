...
基础模型用于验证数据和闭环链路，不代表生产模型精度。当前版本是可演示原型和现场数据接入准备基础，不是生产验收状态。批准范围、旧模型隔离、测量可靠性门禁、Dürr 设备/轨迹血缘和材料治理基线已完成；生产上线前仍必须用工厂批准的真实材料方法/单位/TDS/COA 替换演示占位资料，并完成真实 BYK/Fischer 与 Dürr DXQ/PLC 文件适配、测量不确定度、真实轨迹坐标/姿态/触发和沉积贡献验证、真实历史数据的按生产事件/时间分组验证、工厂约束配置、灰度评估和模型治理。
