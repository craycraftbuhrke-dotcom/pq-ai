...
基础模型用于验证数据和闭环链路，不代表生产模型精度。当前版本是可演示原型和现场数据接入准备基础，不是生产验收状态。批准范围、旧模型隔离和测量可靠性门禁基线已完成；生产上线前仍必须完成真实设备文件适配与测量不确定度、Dürr 轨迹追溯、真实历史数据的按生产事件/时间分组验证、工厂约束配置、灰度评估和模型治理。
