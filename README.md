...
- AI 模型训练、对比、预测、诊断、约束推荐、审批、执行和复测评价工作台
...
- 完整的认证系统: 密码登录、注册、个人中心、角色管理、安全管理
- MySQL Docker 环境、幂等演示数据初始化（DDL 通过 DBA 工单执行）

基础模型用于验证数据和闭环链路，不代表生产模型精度。当前版本是可演示原型和现场数据接入准备基础，不是生产验收状态。批准范围、旧模型隔离、测量可靠性门禁、Dürr 设备/轨迹血缘、材料治理、无泄漏模型验收、适用范围/OOD 阻断和工厂验收策略版本基线已完成；生产上线前仍必须用工厂批准的真实材料方法/单位/TDS/COA 和模型验收阈值替换演示占位资料，并完成真实 BYK/Fischer 与 Dürr DXQ/PLC 文件适配、测量不确定度、真实轨迹坐标/姿态/触发和沉积贡献验证、工厂约束配置和灰度评估。
