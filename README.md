...
```text
API_URL=由运行时注入的后端内网 API 地址，例如 https://your-api.example.com/api/v1
NEXT_PUBLIC_API_URL=由构建时和运行时注入的浏览器可见 API 地址
API_KEY=由运行时 Secret 注入的服务端调用密钥
API_AUTH_ENABLED=true
BOOTSTRAP_ADMIN_PASSWORD=首次演示管理员密码
```
...
API Key。分别配置高熵的 `BOOTSTRAP_ADMIN_PASSWORD`、`BOOTSTRAP_API_KEY` 与 Next.js
服务端 `API_KEY`，并在首次创建正式管理员和正式系统密钥后轮换引导凭据。
