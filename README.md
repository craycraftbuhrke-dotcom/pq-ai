...
- [AI 训练与现场使用路线](docs/ai-training-and-use-route.md)
...
- [新增 7 张训练/远程表独立建表 SQL](docs/sql/pq_ai_training_remote_7_tables.sql)
- [训练双来源既有表兼容升级 SQL](docs/sql/pq_ai_dataset_split_member_training_source_delta.sql)
- [PQRP/1 上位机参数交换协议](docs/pq-upper-computer-protocol.md)
- [3C3B 工程闭环 DBA Delta SQL](docs/sql/pq_ai_3c3b_engineering_delta.sql)
- [环境变量与认证输入清单](docs/environment-variables.md)
- [CodeRabbit 226 项候选问题优化与修复计划](docs/coderabbit-remediation-plan.md)
...
tools/pq-upper-agent/ 无外部运行时的 Windows 上位机代理
docs/           架构与开发说明
```
...
当前完整建表审批材料见 [docs/sql/pq_ai_mysql_schema.sql](docs/sql/pq_ai_mysql_schema.sql)；已有库只补本轮能力时，分别使用 [7 张新增表建表单](docs/sql/pq_ai_training_remote_7_tables.sql) 和 [训练成员表兼容升级单](docs/sql/pq_ai_dataset_split_member_training_source_delta.sql)。这些 SQL 不得由应用、
...
- 自动比较稳健线性/关键参数筛选候选模型，最终验证集不参与选型；支持真实点位预测和相关性贡献诊断
- 模型人工验收、激活门禁、版本激活/退役、在线特征与预测效果漂移监控
- 模型工厂/车型/颜色适用范围、统计 OOD 策略、推理前门禁和预测治理证据
- 不可变训练数据集快照、按车身/生产运行分组的时间留出、独立验证指标和泄漏检查
- 生产点位特征与人工上传训练宽表是同等训练来源，可单独或混合选择；中文 Excel/CSV 模板在后端转换为统一特征矩阵
- 点位页按工厂/车型/颜色/输入分布自动筛选可用模型；推荐目标留空时自动采用匹配度最高的生效质量标准
- 2D 点位详情可同时查看膜厚、色差、橘皮、五站刷子贡献和设定/实绩参数，并直接执行质量预测、关联诊断和约束参数建议
- 任意参数保存均由后端复制全部刷子、全部参数和点位贡献，生成独立完整草稿版本，不修改原版本
- 云端、虚拟生产线、上位机三方完整刷子表快照与差异对账，以及申请、审批、远程暂存、现场确认、正式提交、回读核验状态机
- 目标上位机轻量代理可构建为单个 Windows `.exe`，目标机不需安装 Node.js/Electron/Python/Java，也不会联网下载依赖
...
NEXT_PUBLIC_API_URL=构建时注入的浏览器可见 API 地址
AUTH_ENABLED=true（运行时注入，服务端代理强制开启）
NEXT_PUBLIC_AUTH_ENABLED=true（构建时注入，生产镜像强制开启）
```
...
批量模板覆盖边界与禁止通用覆写的系统表清单见
[docs/bulk-resource-governance.md](docs/bulk-resource-governance.md)。
...
GitLab 流水线提供相互独立的 `backend:deploy` 和 `frontend:deploy` 手动生产部署任务，模板位于
[`deploy/k8s`](deploy/k8s)。目标 namespace 必须预先提供运行参数 ConfigMap、数据库连接 Secret
和前端运行时资产 RWX PVC；仓库和镜像不保存数据库密码，也不会创建数据库或执行 DDL。
镜像内置底图和模型是只读发布资产，PVC 只保存用户上传文件及覆盖清单，滚动发布不会把镜像资源并发复制到共享卷。
...
生产环境必须启用 `API_AUTH_ENABLED=true`、`AUTH_ENABLED=true` 和 `NEXT_PUBLIC_AUTH_ENABLED=true`。网页登录使用用户名/密码换取后端会话令牌，Next.js 只把令牌保存为 HttpOnly Cookie。MES、QMS、机器人/PLC 和脚本集成使用由安全管理页面或企业密钥系统单独签发和轮换的 API Key；人员页面和 Next.js 代理不使用共享 API Key。
