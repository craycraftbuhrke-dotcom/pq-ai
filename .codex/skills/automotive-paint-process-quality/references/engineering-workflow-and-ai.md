...
The implemented PQ-AI acceptance baseline freezes an immutable dataset snapshot containing point-feature values, target values, source quality-measurement IDs, body/run group membership, train/validation split, temporal cutoff, and leakage-check result. The model service fits only `TRAIN`, reports independent `VALIDATION` metrics, creates a `DRAFT` model, generates multi-axis validation folds (temporal holdout, production-group leave-one-out, and factory/model/color axes when diversity exists), registers a model artifact hash, derives pending exact factory/model/color applicability scopes, and creates a pending statistical OOD blocking policy. Human acceptance activates model-governance facts only when every applicable factory has an active target-metric acceptance policy and the model satisfies validation RMSE, validation R², minimum independent-group thresholds, available-axis validation thresholds, and artifact hash integrity. Prediction and recommendation block unsupported, incomplete, and distribution-outlier input while persisting the evidence. Real factory-run evidence, richer uncertainty, external artifact storage, and causal evidence are still mandatory before production use.
