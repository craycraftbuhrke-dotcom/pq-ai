...
The implemented PQ-AI acceptance baseline freezes an immutable dataset snapshot containing point-feature values, target values, source quality-measurement IDs, body/run group membership, train/validation split, temporal cutoff, and leakage-check result. The model service fits only `TRAIN`, reports independent `VALIDATION` metrics, creates a `DRAFT` model, derives pending exact factory/model/color applicability scopes, and creates a pending statistical OOD blocking policy. Human acceptance activates both model-governance facts only when every applicable factory has an active target-metric acceptance policy and the model satisfies its validation RMSE, validation R², and minimum independent-group thresholds. Prediction and recommendation block unsupported, incomplete, and distribution-outlier input while persisting the evidence. Real factory-run evidence, richer uncertainty, and governed artifacts are still mandatory before production use.
